...
export default function ListGuests() {
  ...
  useEffect(() => {
    (async () => {
      const result = await apiRequest(`/guests/get-guests`, {
        method: 'GET'
      });
      setGuests(result)
    })()
  ...
  async function handleUpdateGuest(gue) {
    const { id, name, cpf, birth_date, disability, url_document_picture, url_medical_report } = gue;
    try {
      const result = await apiRequest(`/guests/update-guest/${id}`, {
        method: 'PATCH',
        body: JSON.stringify({
    ...
    <section className="w-full xl:p-20 pr-2 overflow-y-auto">
      {
        ...
        <section className="w-full pr-2 overflow-y-auto">
          ...
          <div className="flex gap-12 items-end mb-8 flex-wrap">
            <Text heading="h1">Atualizar convidados</Text>
          </div>
                      ...
                      <div className="flex flex-col gap-8 mb-8">
                        ...
                        <div className="flex gap-10 lg:gap-15 flex-wrap flex-col lg:flex-row">
                          <div className="flex flex-col w-80 gap-2">
                            ...
                            <MonthYearCalendar
                              date={gue.birth_date ? new Date(gue.birth_date) : new Date("2000-01-01")}
                              setDate={(newDate) => updateGuest(index, "birth_date", newDate?.toISOString())}
                            />
                          </div>
                          <FileUploadBlock
                      ...
                      <div className="flex items-center justify-end w-full space-x-8">
                        <Button variant={'default'} onClick={async () => {
                          if (validarCpf(gue.cpf)) {
                            await handleUpdateGuest(gue);
                          } else {
                            toast.error(`CPF Inválido para dependente ${gue.name}`)
                          }
                        }}>Salvar</Button>
                      </div>
                    </CardContent>
