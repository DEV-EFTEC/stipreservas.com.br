...
export default function ApproveDocumentsDraw() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const response = await apiRequest(`/draws/get-draw-complete?draw_apply_id=${booking.id}`);
      if (response) {
  ...
  useEffect(() => {
    ...
    if (status.status === 'neutral') {
      ...
      setFinalPath(`/admin/sorteio/enviar-aprovacao/${booking.id}`);
    } else if (status.status === 'refused') {
      toast.error(status.message, {
        description: "Está permitido o procedimento de recusa da reserva."
      });
      setFinalPath(`/admin/sorteio/enviar-recusa/${booking.id}`);
    }

  }, [booking]);

  async function handleSubmit() {
    await apiRequest(`/draws/${booking.id}/update-draw-apply`, {
      method: "PUT",
      body: JSON.stringify({
