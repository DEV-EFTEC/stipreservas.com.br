...
export async function createDependentByBooking(req, res) {
  try {
    await dependentsServices.createDependentByBooking(req.body),
      res
        .status(200)
        .json({ message: "dependent_booking atrelado com sucesso!" });
  } catch (err) {
...
export async function findDependentsByUser(req, res) {
  try {
    const user = req.user;
    const dependents = await dependentsServices.findDependentsByUser(user.id);
...
export async function findDependentsByUserAdmin(req, res) {
  try {
    const { user } = req.body;
    const dependents = await dependentsServices.findDependentsByUser(user.id);
    res.status(200).json(dependents);
...
export async function getDependetByParcialName(req, res) {
  try {
    ...
    const dependents = await dependentsServices.getDependetByParcialName(
      parcial_name,
      created_by
    );
    res.status(200).json({ dependents });
